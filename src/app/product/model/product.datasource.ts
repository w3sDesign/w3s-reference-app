...
export class ProductDataSource implements DataSource<any> {
  ...
  /** Accepting/emitting product arrays. */
  ...
  /** Subject for accepting and emitting the total number of queried products. */
  totalNumberOfItems = new BehaviorSubject<number>(0);

  /** Needed e.g. for showing message: 'No products found' */
  hasItems = false;
   ...
   * The observable emits the products that should be displayed.
   ...
   * ##################################################################
   * Implemented by delegating to the product service.
   * If the data arrives successfully, the new values (products) are
  ...
  getProducts(queryParams: QueryParams) {
    ...
    // Delegating to the product service.
    ...
    this.productService.getProducts(queryParams).pipe(
      ...
      switchMap((res1: QueryResult) => {
        ...
        // The product service implementation sets clientSideQuerying to
        ...
        if (queryParams.searchTerm) {
          ...
          this.logMessage(
            `[getProducts(queryParams) 1/3 searchTerm] queryParams = \n ${JSON.stringify(queryParams)}`
          );
          ...
          this.logMessage(
            `[getProducts(queryParams) 2/3 filter] queryParams = \n ${JSON.stringify(queryParams)}`
          );
          this.logMessage(
            `[getProducts(queryParams) 2/3 filter] res1.items = \n ${JSON.stringify(res1.items)}`
          );
          ...
          this.logMessage(
            `[getProducts(queryParams) 3/3 sort] queryParams = \n ${JSON.stringify(queryParams)}`
          );
          ...
          // ==============================================================
          // Product service returns a QueryResult observable.
  ...
  private logMessage(message: string) {
    return this.messageService.logMessage('[########## product.datasource.ts ##########] ' + message);
  }
